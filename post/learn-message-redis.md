...
Redis Cluster 会因为负载均衡或节点故障等原因而执行数据迁移，而这就会导致客户端访问的 key 并不在接收到命令的集群节点上, 需要客户端重定向， key所在的slot正在迁入迁出时，key不在当前节点也会导致发送MOVED指令到客户端<br>
而 CLUSTER_SLOTS 宏定义的值是 16384，表示的是 Redis Cluster 的 slots 总个数<br>
...
选择 16384 个哈希槽（即 (2^{14})）的原因主要基于以下核心权衡
1. 集群元数据大小优化, 心跳包结构/位图表示法, (最终选择 16384（(2^{14})），仅需 2KB，平衡了网络效率和内存开销)
2. CRC16 算法生成 16位哈希值（范围 `0-65535`），但 Redis 仅取模 16384
3. 哈希冲突概率
